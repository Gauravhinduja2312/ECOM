...
export default function OrderTrackingCard({ order, isExpandable = true }) {
  ...
  return (
    <div className="group rounded-[2rem] border border-white/5 bg-slate-950/50 backdrop-blur-xl shadow-sm transition-all duration-300 hover:shadow-xl hover:shadow-indigo-500/5">
      {/* Order Header */}
      <div
        ...
        <div className="flex flex-wrap items-center justify-between gap-6">
          <div className="flex-1">
            <div className="flex items-center gap-4 flex-wrap">
              <span className="text-xl font-black tracking-tighter text-white uppercase">Order #{order.id}</span>
      ...
      {isExpanded && (
        <div className="px-6 pb-8 sm:px-8 stagger-children">
          {/* Status Timeline */}
          <div className="rounded-3xl bg-white/5 border border-white/5 p-6 mb-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {/* Pickup Details */}
            <div className="rounded-3xl border border-white/5 p-6 hover:bg-white/5 transition text-white">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-white">{order.pickup_location || 'To be confirmed'}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-white">{pickupDateTime}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Order Items */}
            <div className="rounded-3xl border border-white/5 p-6 hover:bg-white/5 transition">
              ...
              <div className="space-y-3">
                {order.items?.map((item) => (
                  <div key={item.id} className="flex items-center justify-between">
                    ...
                    <p className="text-xs font-black text-white">{formatCurrency(Number(item.price) * Number(item.quantity))}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="mt-8">
            <button
              ...
              className="w-full btn-primary py-4 text-xs tracking-widest uppercase"
            >
